...
public class Neo4jSeasonRateController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Neo4jSeasonRate> getById(@PathVariable Long id) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Neo4jSeasonRate> update(@PathVariable Long id, @RequestBody Neo4jSeasonRate seasonRate) {
        return service.findById(id).map(existing -> {
            seasonRate.setRateId(id);
